functions {
	...
	real expected_value(real mu, real sigma, int ymax) {
	    real prob;
	    real ev = 0.0;
	    for (j in 1:ymax) {
	    ...
	    ev += ymax * (1 - normal_cdf(ymax, mu, sigma));
	    return ev;
	}
}
data {
...
model {
    for (j in 1:N) {
        for (i in 1:5) {
            if (w[j][i] > 0) {
                target += w[j][i] * log_lik_fn(y[j][i], obs[j][i],  mu[mu_id[j]][i] + batch_eff[j], sigma);
            }
        }
    }
    alpha0 ~ normal(0, 3);
...
generated quantities {
    ...
    for (j in 1:N) {
        for (i in 1:Nt) {
        	ev[j][i] = expected_value(mu[mu_id[j]][i] + batch_eff[j], sigma, 10);
        }
    }
}